...
switch($_GET['act'])
{
	...
	$db->execute("update `bixos` set `hp`=0, `type`=? where `player_id`=?", array(99, $player->id));
		...
		if (!$_GET['nolayout']){ $player = check_user($secret_key, $db); include("templates/private_header.php"); }
		...
		echo "<div id=\"logdebatalha\" class=\"scroll\" style=\"background-color:#FFFDE0; overflow: auto; height:220px; padding:5px; border: 1px solid #DEDEDE; margin-bottom:10px\">";
			...
			$log = explode(", ", $log);
				if (($log[0] == 1) or ($log[0] == 3) or ($log[0] == 5)) { echo "<div style=\"text-align: left\">"; }else{ echo "<div style=\"text-align: right\">"; }
					if ($log[0] == 1){ echo "<font color=\"green\">"; } elseif ($log[0] == 2){ echo "<font color=\"red\">"; } elseif ($log[0] == 3){ echo "<font color=\"blue\">"; } elseif ($log[0] == 4){ echo "<font color=\"purple\">"; } else { echo "<font color=\"black\">"; }
						echo $log[1];
					echo "</font>";
				echo "</div>";
			}
