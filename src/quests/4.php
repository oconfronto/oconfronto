...
if (($missao['quest_status'] ?? null) == 1) {
    ...
    $db->execute("update `quests` set `quest_status`='3', `extra`=? where `id`=?", [time() + 900, $missao['id'] ?? null]);
    ...
    if (time() > ($missao['extra'] ?? null)) {
        $db->execute("update `quests` set `quest_status`='4' where `id`=?", [$missao['id'] ?? null]);
        header("Location: tavern.php?p=quests&start=" . $quest['id'] . "");
        exit;
    }
...
} elseif (($missao['quest_status'] ?? null) == 4) {
    if (($_GET['talk'] ?? null) == 1) {
    ...
    } elseif (($missao['pago'] ?? null) == 't') {
        $db->execute("delete from `items` where `item_id`=116 and `player_id`=? limit 1", [$player->id]);
        $db->execute("update `quests` set `quest_status`='5', `extra`=? where `id`=?", [time() + 900, $missao['id'] ?? null]);
...
} elseif (($missao['quest_status'] ?? null) == 5) {
    if (time() > ($missao['extra'] ?? null)) {
        $db->execute("update `quests` set `quest_status`='6' where `id`=?", [$missao['id'] ?? null]);
        header("Location: tavern.php?p=quests&start=" . $quest['id'] . "");
        exit;
    }
    ...
    $b = '<a href="home.php">Principal</a>';
} elseif (($missao['quest_status'] ?? null) == 6) {
    $db->execute("update `quests` set `quest_status`='7' where `id`=?", [$missao['id'] ?? null]);

    $a = "<i>Você chegou na cidade.</i>";
    $b = '<a href="tavern.php?p=quests&start=' . $quest['id'] . '">Falar com Trevus</a>';
} elseif (($missao['quest_status'] ?? null) == 7) {
