...
if ((60 - $backpackquery->recordcount()) >= 0) {
	echo (60 - $backpackquery->recordcount());
} else {
	echo "0";
}
...
while ($bag = $backpackquery->fetchrow()) {
	...
	if (($bag['item_bonus'] ?? null) > 2 && ($bag['item_bonus'] ?? null) < 6) {
		$colorbg = "itembg2";
	} elseif (($bag['item_bonus'] ?? null) > 5 && ($bag['item_bonus'] ?? null) < 9) {
		$colorbg = "itembg3";
	} elseif (($bag['item_bonus'] ?? null) == 9) {
		$colorbg = "itembg4";
	} elseif (($bag['item_bonus'] ?? null) > 9) {
		$colorbg = "itembg5";
	} else {
	...
	if (($bag['type'] ?? null) == 'amulet') {
		$nametype = "Vitalidade";
	} elseif (($bag['type'] ?? null) == 'weapon') {
		$nametype = "Ataque";
	} elseif (($bag['type'] ?? null) == 'boots') {
		$nametype = "Agilidade";
	} else {
		$nametype = "Defesa";
	}

	if (($bag['type'] ?? null) != 'addon' && ($bag['type'] ?? null) != 'ring') {
	...
	$need = NULL;
	if (($bag['needlvl'] ?? null) > 1) {
		$lvlbonus = $player->vip > time() ? 10 : 0;

		if (($bag['needlvl'] ?? null) > ($player->level + $lvlbonus)) {
...
echo '<div id="btn_mobile" style="display: none; justify-content: center; align-items: center;padding:20px;text-align:center">';
echo "<br />";
echo "<br />";
echo "<a id=\"link\" class=\"neg\" style='color:#fff;' href=\"inventory_mobile.php\">Inventário Mobile</a>";
echo "<br />";
echo "<br />";
echo "</div>";
