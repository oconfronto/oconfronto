...
if ($_GET['act'] == cancel){
    ...
    echo "<br/><p><center>A solicitaÁ„o para mudanÁa de email foi removida. <a href=\"characters.php\">Voltar</a>.</center></p><br/>";
...
if ($_POST['submit']) {
    if (!$_POST['senhadaconta']) {
        ...
        $errmsg .= "VocÍ n„o digitou os dois emails corretamente!";
        $error = 1;
    } else if (strlen($_POST['emaill']) < 3) {
        $errmsg .= "O seu endereÁo de email deve conter mais de 5 caracteres.";
        $error = 1;
    } else if (strlen($_POST['emaill']) > 200) {
        $errmsg .= "O seu endereÁo de email deve conter menos de 200 caracteres.";
        $error = 1;
    } else if (!preg_match("/^[-!#$%&\'*+\\.\/0-9=?A-Z^_`{|}~]+@([-0-9A-Z]+\.)+([0-9A-Z]){2,4}$/i", $_POST['emaill'])) {
        $errmsg .= "O formato do seu email È inv·lido!";
        $error = 1;
    } else {
        ...
        if ($query->recordcount() > 0) {
            ...
            $errmsg .= "VocÍ j· enviou uma solicitaÁ„o de mudanÁa de email.";
            $error = 1;
        }
    ...
    if ($error == 0) {
        ...
        echo "<span id=\"aviso-a\"></span>";
        echo "<br/><p><center>Seu email será alterado para: " . $_POST['emaill'] . ".<br/>Aguarde 14 dias para que a mudança seja efetuada. <a href=\"characters.php\">Voltar</a>.</center></p><br/>";
        include("templates/acc-footer.php");
        exit;
    }
}
