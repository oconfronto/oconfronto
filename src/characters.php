...
include(__DIR__ . "/templates/acc-header.php");
	...
	if($queryactivate1 = $db->execute("select * from `players_ref` where `id_p_c`=? ", array($acc->id))){;
		...
		if($type_item > 0){
			...
			$insert_item_for = rand(1,5);
			$insert_item_vit = rand(1,5);
			$insert_item_agi = rand(1,5);
			$insert_item_res = rand(1,5);
			...
			$id = $db->Insert_ID();
			$status = $db->execute("update `items` set `for`=`for`+?, `vit`=`vit`+?, `agi`=`agi`+?, `res`=`res`+? where `id`=?", array($insert_item_for, $insert_item_vit, $insert_item_agi, $insert_item_res, $id));
		}	
	...
	$query7 = $db->execute("update `players` set `gold`=`gold`+?, `ref`=`ref`+1 where `id`=?", array($setting->earn, $acc->ref));
    $validaconta = $db->execute("update `accounts` set `ref`='t' where `id`=?", array($acc->id));
	$validaconta = $db->execute("update `player_ref` set `session_id`='2' where `id`=?", array($acc->id));
