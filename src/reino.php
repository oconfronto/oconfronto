...
if ((($reino['poll'] + 604800) < time()) and ($reino['imperador'] > 0)) {
	$db->execute("update `reinos` set `imperador`=0, `poll`=? where `id`=?", array((time() + 172800), $reino['id']));
...
include("templates/private_header.php");
            ...
            if ($reino['imperador'] == 0){
                if ($_POST['vote']){
                        $verifica = $db->execute("select * from `reino_votes` where `player_id`=?", array($player->id));
                        if ($verifica->recordcount() == 0){
                            $votes = $db->execute("select * from `reino_tovote` where `player_id`=?", array($_POST['vote']));
	...
	echo "<td width=\"80%\">";
		...
		echo "<table width=\"100%\">";
			...
			echo "<tr class=\"salmon\"><td>";
				...
				echo "<table width=\"100%\">";
				$query = $db->execute("select `id`, `kills`, `akills`, `level` from `players` where `reino`=? and `gm_rank`<10", array($player->reino));
...
echo "<table width=\"100%\">";
	...
	if ($reino['imperador'] != 0){
		...
		if (($reino['imperador'] == 0) and (time() > $reino['poll'])){
			$total = $db->execute("select `vote_id` from `reino_votes` where `reino_id`=? group by `vote_id` order by count(*) desc limit 1", array($reino['id']));
			$total = $total->fetchrow();

			$db->execute("update `reinos` set `imperador`=? where `id`=?", array($total['vote_id'], $reino['id']));
			$db->execute("delete from `reino_votes` where `reino_id`=?", array($reino['id']));
			$db->execute("delete from `reino_tovote` where `reino_id`=?", array($reino['id']));
		...
		if ($votes->recordcount() == 0){
			$query = $db->execute("select `id` from `players` where `reino`=? order by `uptime`+(`posts` * 420)+(`akills` * 30)-(`kills` * 10) desc limit 5", array($reino['id']));
			while ($member = $query->fetchrow())
			{
			...
			$db->execute("update `reinos` set `poll`=? where `id`=?", array(time() + 172800, $reino['id']));
			$query = $db->execute("select * from `reinos` where `id`=?", array($player->reino));
			$reino = $query->fetchrow();
		}
