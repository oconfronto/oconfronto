...
if ($_POST['days'] > 998){
$ban = $db->execute("update `players` set `alerts`=? where `id`=?", array(forever, $_GET['player']));
...
$ban = $db->execute("update `players` set `alerts`=? where `id`=?", array($totaldias, $_GET['player']));
