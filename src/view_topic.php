...
if (($_GET['up']) or ($_GET['down']))
{
	...
	if ($jaupou->recordcount() > 0){
		echo showAlert("Você já votou neste tópico!", "red");
	} else {
...
if ($_POST['a_answer']) {
	...
	if ($fecxhado['closed'] == 't') {
		echo showAlert("Este tópico está fechado.", "red");

	} elseif ($foruminfo->recordcount() != 1) {
		echo showAlert("Este tópico não existe.", "red");

	} elseif ((($categoryae == 'gangues') or ($categoryae == 'trade')) and ($player->serv != $servae)){
		echo showAlert("Você não pode postar aqui.", "red");
	
	} elseif ($lastpost->recordcount() != 0) {
		echo showAlert("Não faça postagens seguidas.<br/>Aguarde 20 segundos para poder postar novamente.", "red");
...
echo "<b><font size=\"1px\"><a href=\"select_forum.php\">Fóruns</a> -> <a href=\"main_forum.php?cat=" . $rows['category'] . "\">" . ucfirst($categoria) . "</a> -> <a href=\"view_topic.php?id=" . $rows['id'] . "\">" . ucfirst(stripslashes($rows['topic'])) . "</a></font></b>";
