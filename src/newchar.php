...
if ($_POST['register'])
{
	...
	else if (strlen($nomedeusuari0) > 15)
	{ //If username is too short...
	...
	if ($error == 0)
	{
		...
		{
            echo "<span id=\"aviso-a\"></span>";
			echo "<br/><p><center>Seu personagem foi criado com sucesso!<br />";
			echo "<a href=\"login.php?id=" . $player['id'] . "\">Clique aqui</a> e começe a jogar com " . $nomedeusuario . ".</center></p><br />";
            include("templates/acc-footer.php");
  			exit;
		}
	}
...
<table width="90%" align="center" border=\"0px\">
    <tr>
        ...
        <td width="72%"><input type="text" name="username" id="username" value="<?=$_POST['username'];?>" class="inp" size="20"/><span id="msgbox"><?php 
        if ($erro1 == 1){
    ...
    <tr>
<td width="28%"><b>Vocação</b>:</td><td width="72%"><select name="voc" onchange="swapText(this)" class="inp"><option value="none" selected="selected">Selecione</option><option value="knight">Guerreiro</option><option value="mage">Mago</option><option value="archer">Arqueiro</option></select><?php 
    if ($erro2 == 1){
        echo "<span id=\"erro\"></span>";
    } else if ($certo2 == 1){
        echo "<span id=\"certo\"></span>";
    ...
    <tr><td colspan="2"><font size="1px"><div id="textDiv">Escolha sua vocação.</div></font></td>
    </tr>
</table>
<br/>
