...
if ($player->username != ($guild['leader'] ?? null) && $player->username != ($guild['vice'] ?? null)) {
    ...
    if (($_GET['upgrade'] ?? null) == "maxplayers") {
        if (($guild['maxmembers'] ?? null) > 29) {
            $errmsg .= "<center><b>Você não pode adicionar mais vagas para o clã.</b></center>";
            $error = 1;
        } elseif ($price > ($guild['gold'] ?? null)) {
            $errmsg .= "<center><b>Seu clã não possui ouro suficiente. (Preço: " . $price . ")</b></center>";
            $error = 1;
        }

        if ($error == 0) {
            $query = $db->execute("update `guilds` set `maxmembers`=?, `gold`=? where `id`=?", [$guild['maxmembers'] + 5, $guild['gold'] - $price, $guild['id'] ?? null]);
...
?>
    ...
    <fieldset>
        <legend><b><?php echo $guild['name'] ?? null; ?> :: Melhorias</b></legend>
        <br />
        ...
        <br />
        <center><i>O clã possui atualmente <?php echo $guild['maxmembers'] ?? null; ?> vagas.</i></center>
    </fieldset>
