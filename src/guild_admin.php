...
if (($_POST['price'] ?? null) && ($_POST['submit'] ?? null)) {
    if (($_POST['price'] ?? null) < 0) {
        $msg1 .= "<font color=\"red\">O preço para entrar no clã deve ser 0 ou mais.</font><p />";
        $error = 1;
    } elseif (($_POST['price'] ?? null) == ($guild['price'] ?? null)) {
        $error = 1;
    } elseif (($_POST['price'] ?? null) > 999999) {
        ...
        $query = $db->execute("update `guilds` set `price`=? where `id`=?", [$_POST['price'] ?? null, $guild['id'] ?? null]);
...
if ($_POST['upload'] ?? null) {
    if (!($_POST['guild_admin'] ?? null)) {
        $errmsg .= "Por favor preencha todos os campos!";
        $error = 1;
    } elseif (($_POST['guild_admin'] ?? null) && (@GetImageSize($_POST['guild_admin']) === [] || @GetImageSize($_POST['guild_admin']) === false)) {
        $errmsg .= "O endereço desta imagem não é válido!";
        $error = 1;
    }

    if ($error == 0) {
        $avat = $_POST['guild_admin'] ?? null ?: "default_guild.png";
        $query = $db->execute("update `guilds` set `img`=? where `id`=?", [$avat, $guild['id'] ?? null]);
...
if (($_POST['motd'] ?? null) && ($_POST['submit'] ?? null)) {
    if (strlen((string) ($_POST['motd'] ?? null)) < 3) {
        $msg3 .= "<font color=\"red\">A mensagem do seu clã deve conter de 3 é 220 caracteres!</font><p />";
        $error = 1;
    } elseif (($_POST['motd'] ?? null) == ($guild['motd'] ?? null)) {
        $error = 1;
    } elseif (strlen((string) ($_POST['motd'] ?? null)) > 220) {
        $msg3 .= "<font color=\"red\">A mensagem do seu clã deve conter de 3 é 220 caracteres!</font><p />";
        $error = 1;
    } else {
        $query = $db->execute("update `guilds` set `motd`=? where `id`=?", [$_POST['motd'] ?? null, $guild['id'] ?? null]);
        $msg3 .= "Você trocou a mensagem do seu clã.<p />";
    }
}

//If blurb set then update query
if (($_POST['blurb'] ?? null) && ($_POST['submit'] ?? null)) {
    if (strlen((string) ($_POST['blurb'] ?? null)) < 50) {
        $msg4 .= "<font color=\"red\">A descrição deve ser maior que 50 caracteres!</font><p />";
        $error = 1;
    } elseif (($_POST['blurb'] ?? null) == ($guild['blurb'] ?? null)) {
        $error = 1;
    } elseif (strlen((string) ($_POST['blurb'] ?? null)) > 5000) {
        ...
        $query = $db->execute("update `guilds` set `blurb`=? where `id`=?", [$texto, $guild['id'] ?? null]);
...
<fieldset>
    ...
    <table width="100%">
        <thead>
            <tr>
                <div style="text-align: center;"><img src="<?php $dire = (($guild['img'] ?? null) == "default_guild.png") ? "static/" . $guild['img'] : $guild['img'] ?? null;
                                                            echo $dire ?>" width="120px" height="120px"
                        alt="<?php echo $guild['name'] ?? null ?>" border="1px"></div>
            </tr>
        </thead>
        <tbody>
            <tr>
                <form action="guild_admin.php" method="POST">
                    ...
                    <td>
                        <input type="text" name="guild_admin" value="<?= $guild['img'] ?? null ?>" size="45" />
            ...
            <form method="POST" action="guild_admin.php">
                <tr>
                    ...
                    <td><input type="text" name="price" value="<?php
                                                                if (!($_POST['price'] ?? null)) {
                                                                    echo $guild['price'] ?? null;
                                                                } else {
                                                                    echo $_POST['price'] ?? null;
                                                                }
                ...
                <tr>
                    ...
                    <td><input type="text" name="motd" size="40" value="<?php
                                                                        if (!($_POST['motd'] ?? null)) {
                                                                            echo $guild['motd'] ?? null;
                                                                        } else {
                                                                            echo $_POST['motd'] ?? null;
                                                                        }
                ...
                <tr>
                    ...
                    <td>
                        <?php
                        $textoreferencia = $_POST['blurb'] ?? null ?: $guild['blurb'] ?? null;
                        ?>
                        <script>
