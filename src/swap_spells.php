...
if ($_GET['estender'] ?? null) {
	...
	if (($_GET['spell'] ?? null) && is_numeric($_GET['spell'])) {
		$getid = (int)($_GET['spell'] ?? null);  // Convert directly to integer for spell ID
	...
	if (($_GET['spell'] ?? null) && ($_GET['confirm'] ?? null) != 'yes' && is_numeric($_GET['spell']) && $magic->recordcount() == 1) {
	...
	} elseif (($_GET['spell'] ?? null) && ($_GET['confirm'] ?? null) == 'yes' && is_numeric($_GET['spell']) && $magic->recordcount() == 1) {
		...
		if (($magia['precisa'] ?? null) != 'f') {
			$nescecita = explode(", ", (string) ($magia['precisa'] ?? null));
			$verifica1 = $db->execute("select * from `magias` where `magia_id`=? and `player_id`=?", [$nescecita[0] ?? null, $player->id]);
			$verifica2 = $db->execute("select * from `magias` where `magia_id`=? and `player_id`=?", [$nescecita[1] ?? null, $player->id]);
			$verifica3 = $db->execute("select * from `magias` where `magia_id`=? and `player_id`=?", [$nescecita[2] ?? null, $player->id]);
			$verifica4 = $db->execute("select * from `magias` where `magia_id`=? and `player_id`=?", [$nescecita[3] ?? null, $player->id]);
			$verifica5 = $db->execute("select * from `magias` where `magia_id`=? and `player_id`=?", [$nescecita[4] ?? null, $player->id]);
		...
		if (($magia['cost'] ?? null) > $player->magic_points) {
