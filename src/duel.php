...
if ($_GET['start'])
{
    $verificaLuta = $db->execute("select `id` from `duels` where `status`!='w' and `status`!='z' and (`p_id`=? or `e_id`=?)", array($player->id, $player->id));
    if ($verificaLuta->recordcount() > 0) {
        ...
        $checkDuelStart = $db->execute("select `id` from `duels` where `status`='w' and `id`=? and (`p_id`=? or `e_id`=?)", array($_GET['start'], $player->id, $player->id));
        if ($checkDuelStart->recordcount() > 0) {
            $db->execute("update `duels` set `status`='t' where `id`=?", array($_GET['start']));
            if ($_GET['nolayout']) {
                header("Location: duel.php?luta=true&new=true&nolayout=true");
            } else {
                header("Location: duel.php?luta=true&new=true");
...
if (($_GET['luta']) or ($verificaLuta->recordcount() > 0))
{
    ...
    if ($luta['status'] == 'z') {
        if ($_GET['new']) {
            $db->execute("delete from `duels` where `id`=?", array($luta['id']));
    ...
    {
        $getEnemy = $db->execute("select * from `players` where `id`=?", array($luta['e_id']));
    } else {
        $getEnemy = $db->execute("select * from `players` where `id`=?", array($luta['p_id']));
    }
    ...
    if ($luta['status'] == 't') {
        $db->execute("update `duels` set `status`=?, `timeout`=? where `id`=?", array($enemy->id, time() + 45, $luta['id']));
        $verificaLuta = $db->execute("select * from `duels` where `id`=?", array($luta['id']));
        $luta = $verificaLuta->fetchrow();
    }
    
    if ((is_numeric($luta['status'])) and ($luta['status'] != $player->id))
    {
        ...
        if ($luta['timeout'] > time()) {
            ...
            $db->execute("update `duels` set `status`='w', `timeout`='0' where `id`=?", array($luta['id']));
            if ($_GET['nolayout']) {
    ...
    } elseif (($luta['status'] != 's') and ($luta['status'] != 'z')) {
        unset($_SESSION['statusduellog']);
        $db->execute("update `duels` set `status`='s', `timeout`=? where `id`=?", array(time() + 30, $luta['id']));
        $verificaLuta = $db->execute("select * from `duels` where `id`=?", array($luta['id']));
        $luta = $verificaLuta->fetchrow();
    }
    ...
    if (($luta['log'] == null) or ($luta['log'] == "")){
        $duellog = array();
    } else {
        $duellog = unserialize($luta['log']);
    }
        
    if (($player->hp > 0) and ($enemy->hp > 0) and ($luta['status'] != 'z'))
    {
        if ((($player->id == $luta['p_id']) and ($luta['vez'] == 'p')) or (($player->id != $luta['p_id']) and ($luta['vez'] == 'e')))
        {
            if ($player->id == $luta['p_id']) {
                $prox = "e";
            } else {
            ...
            elseif ($type == 6){
				...
				if ($checamagiccivo->recordcount() > 0){
                    include("duel/defesatripla.php");
				}else{
                    include("duele/atacahit.php");
				}
			}
			elseif ($type == 7){
            ...
            if (time() >= $luta['timeout'])
            {
                if ($luta['extra'] != $player->id) {
                    ...
                    $db->execute("update `duels` set `extra`=? where `id`=?", array($player->id, $luta['id']));
                    $otroatak = 3;
                } else {
                    array_unshift($duellog, "13, " . $player->username . "");
                    $db->execute("update `duels` set `extra`='0' where `id`=?", array($luta['id']));
			...
			if ($otroatak != 5) {
				...
				$db->execute("update `duels` set `p_type`='0', `e_type`='0', `vez`=?, `log`=?, `timeout`=? where `id`=?", array($prox, serialize($duellog), time() + 30, $luta['id']));
                if ($player->id == $luta['p_id']) {
                    $db->execute("update `duels` set `p_turnos`=`p_turnos`-1 where `p_turnos`>0 and `id`=?", array($luta['id']));
                } else {
                    $db->execute("update `duels` set `e_turnos`=`e_turnos`-1 where `e_turnos`>0 and `id`=?", array($luta['id']));
                }
                $db->execute("update `duels` set `p_magia`='0' where `p_turnos`='0' and `id`=?", array($luta['id']));
                $db->execute("update `duels` set `e_magia`='0' where `e_turnos`='0' and `id`=?", array($luta['id']));
			}
        } else {
            if (time() >= $luta['timeout'])
            {
                if ($luta['extra'] != $enemy->id) {
                    ...
                    $db->execute("update `duels` set `extra`=? where `id`=?", array($enemy->id, $luta['id']));
    ...
    if ($fugiu == 5){
        $db->execute("update `duels` set `p_type`='99', `e_type`='99', `status`='z', `extra`=? where `id`=?", array($player->id, $luta['id']));
        $output .= showAlert("<b>Voc&ecirc; fugiu da luta com sucesso!</b>", "green");
        array_unshift($duellog, "13, " . $player->username . ", " . $enemy->username . "");
    }
    
    if ((($player->hp < 1) or ($morreu == 5)) and ($type != 99)){
            ...
            $db->execute("update `players` set `energy`=`energy`-?, `exp`=`exp`-?, `deaths`=`deaths`+1, `hp`=0, `mana`=0, `deadtime`=? where `id`=?", array(10, $exploss, time() + $setting->dead_time, $player->id));
        if ($player->id == $luta['p_id']) {
            $db->execute("update `duels` set `p_type`='99', `status`='z' where `id`=?", array($luta['id']));
        } else {
            $db->execute("update `duels` set `e_type`='99', `status`='z' where `id`=?", array($luta['id']));
        }
    }
    
    if ((($enemy->hp < 1) or ($matou == 5)) and ($type != 99)){
        ...
        if ($expwin + $player->exp >= maxExp($player->level)) //Player gained a level!
        {
            ...
            $db->execute("update `players` set `magic_points`=`magic_points`+1, `stat_points`=`stat_points`+3, `level`=`level`+1, `hp`=?, `maxhp`=?, `exp`=`exp`+?, `energy`=`energy`-10 where `id`=?", array(maxHp($db, $player->id, $player->level, $player->reino, $player->vip), maxHp($db, $player->id, $player->level, $player->reino, $player->vip), $newexp, $player->id));
        }
        else
        {
            //Update player
            $db->execute("update `players` set `exp`=`exp`+?, `hp`=?, `energy`=`energy`-10 where `id`=?", array($expwin, $player->hp, $player->id));
        }
        ...
        if ($player->id == $luta['p_id']) {
            $db->execute("update `duels` set `p_type`='99', `status`='z', `venceu`=? where `id`=?", array($player->id, $luta['id']));
        } else {
            $db->execute("update `duels` set `e_type`='99', `status`='z', `venceu`=? where `id`=?", array($player->id, $luta['id']));
        }
    }
    
    if (($luta['status'] == 'z') and ($_SESSION['statusduellog'] == null))
    {
    ...
    if (!$_GET['nolayout']){ include("templates/private_header.php"); } else { header("Content-Type: text/html; charset=utf-8",true); }
        ...
        echo "<div id=\"swap\"></div><div id=\"battle\">";
            ...
            if (($luta['status'] != 'z') and ($player->hp > 0) and ($enemy->hp > 0) and ($matou != 5) and ($morreu != 5) and ($luta['p_type'] != '99') and ($luta['e_type'] != '99')){
                        ...
                        echo "<td width=\"39%\" align=\"center\">";
                                echo "<table width=\"100%\" align=\"center\"><tr>";
                                    echo "<td>";
                                        ...
                                        echo show_prog_bar(155, ceil(($enemy->hp / $enemy->maxhp) * 100), $enemy->hp, 'red', '#FFF');
                                        echo "<br />";
                                        echo show_prog_bar(155, ceil(($enemy->mana / $enemy->maxmana) * 100), $enemy->mana, 'blue', '#FFF');
                                        echo "<div>";
        ...
        if (($player->hp < 1) or ($enemy->hp < 1) or ($matou == 5) or ($morreu == 5) or ($luta['p_type'] == '99') or ($luta['e_type'] == '99')){
...
if ($_POST['desafiar'])
{
    $checkEnemy = $db->execute("select `id` from `players` where `username`=?", array($_POST['username']));
	if ($checkEnemy->recordcount() > 0) {
        $getEnemyId = $db->GetOne("select `id` from `players` where `username`=?", array($_POST['username']));
    }
