...
if ($_GET['start'])
{
    ...
    if (!$_GET['nolayout']){ include(__DIR__ . "/templates/private_header.php"); } else { header("Content-Type: text/html; charset=utf-8",true); }

    echo "Desafio não encontrado.<br/><a href=\"duel.php\">Voltar</a>.";
    if (!$_GET['nolayout']){ include(__DIR__ . "/templates/private_footer.php"); }
...
if ($_GET['luta'] || $verificaLuta->recordcount() > 0)
{
    ...
    $duellog = $luta['log'] == null || $luta['log'] == "" ? array() : unserialize($luta['log']);
