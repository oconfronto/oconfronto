...
class OCv2 {
    ...
    function totaldados($data, $data2=false, $data3=true, $vl=false){if(!$vl){$vll='=';}else{$vll='>';}
        if(!$data2 || !$data3){
            $query = mysql_query("SELECT * FROM $data");
            return  mysql_num_rows($query);}else{
...
function showAlert($msg, $color = '#FFFDE0', $align = 'center', $link = NULL, $id = NULL) {
	...
	if ($link){
		...
		$return .= "<div "; if (id) { $return .= "id = \"" . $id . "\" "; } $return .= "class=\"off\" onmouseover=\"this.className='on'\" onmouseout=\"this.className='off'\" style=\"color: #000000; padding: 5px; border: 1px solid #DEDEDE; margin-bottom: 10px; text-align: " . $align . ";\">";
	}else{
