...
switch ($_GET['p'] ?? null) {
    case "quests":
        if ($_GET['start'] ?? null) {
            $query = $db->execute("select * from `allquests` where `id`=?", [$_GET['start'] ?? null]);
            ...
            if ($qStatus == 90) {
            ...
            } elseif (($quest['lvl'] ?? null) > $player->level || ($quest['to_lvl'] ?? null) < $player->level && ($quest['to_lvl'] ?? null) > 0) {
                $a = "Você não possui o nível necessário para esta missão!";
                $b = '<center><a href="tavern.php?p=quests">Voltar</a></center>';
            } elseif ($qStatus > 0) {
                $query = $db->execute("select * from `quests` where `player_id`=? and `quest_id`=?", [$player->id, $quest['id'] ?? null]);
                ...
                if ($_GET['pay'] ?? null) {
                    if ($player->gold - $quest['cost'] < 0) {
                        ...
                        $db->execute("update `players` set `gold`=`gold`-? where `id`=?", [$quest['cost'] ?? null, $player->id]);
                        $db->execute("update `quests` set `pago`='t' where `id`=?", [$missao['id'] ?? null]);
        ...
        while ($quest = $query->fetchrow()) {
            ...
            if (($quest['lvl'] ?? null) > $player->level || ($quest['to_lvl'] ?? null) < $player->level && ($quest['to_lvl'] ?? null) > 0) {
                $q .= '<font color="red">';
                $closered = true;
            }

            if (($quest['to_lvl'] ?? null) > 0) {
        ...
        if ($gettasks->recordcount() < 1) {
            ...
            while ($task = $gettasks->fetchrow()) {
                ...
                if ($checkcompleted->recordcount() == 0) {
                    ...
                    if (($task['needlvl'] ?? null) > $player->level) {
    ...
    case "bar":
        if (($_GET['act'] ?? null) == 'buy') {
            ...
            $bebid = $db->execute("select `id`, `name`, `price`, `effectiveness` from `blueprint_items` where `id`=? and `type`='potion' and `effectiveness`>0", [$_GET['id'] ?? null]);
            ...
            if (($buy['id'] ?? null) == 182 && $bebado->recordcount() == 0) {
                include(__DIR__ . "/templates/private_header.php");
                echo "<b>Taverna:</b><br />\n";
            ...
            $itemprice = $player->reino == '1' || $player->vip > time() ? ceil($buy['price'] * 0.9) : $buy['price'] ?? null;
        ...
        while ($item = $lista->fetchrow()) {
            ...
            if (($item['effectiveness'] ?? null) >= 60) {
                echo " <b>Duração:</b> " . ($item['effectiveness'] / 60) . " hora(s).";
            } elseif (($item['effectiveness'] ?? null) > 1) {
