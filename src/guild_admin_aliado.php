...
<b>Solicitar aliança com o clã:</b> <?php $query = $db->execute("select `id`, `name` from `guilds` where `name`!=? and `serv`=?", array($guild['name'], $guild['serv']));
...
echo "<p /><center>Seu clã não possui alianças.</center><p />";
	...
	while($ali = $query0000->fetchrow()){
		...
		$postgmembers = $db->GetOne("select `members` from `guilds` where `id`=?", array($ali[aled_na]));
