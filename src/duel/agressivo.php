<?php
$selectmana = $db->GetOne("select `mana` from `blueprint_magias` where `id`=2");
...
if ($player->mana < $mana){
    ...
    if ($player->id == $luta['p_id']) {
        $db->execute("update `duels` set `p_magia`='2', `p_turnos`='6' where `id`=?", array($luta['id']));
    } else {
        $db->execute("update `duels` set `e_magia`='2', `e_turnos`='6' where `id`=?", array($luta['id']));
    }
