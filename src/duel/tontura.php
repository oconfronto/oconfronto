...
if ($player->mana < $mana){
    ...
    if ($player->id == $luta['p_id']) {
        $db->execute("update `duels` set `p_magia`='11', `p_turnos`='6' where `id`=?", array($luta['id']));
    } else {
        $db->execute("update `duels` set `e_magia`='11', `e_turnos`='6' where `id`=?", array($luta['id']));
    }
    
    $db->execute("update `players` set `mana`=`mana`-? where `id`=?", array($mana, $player->id));
    array_unshift($duellog, "3, " . $player->username . ", tontura");
}
?>
