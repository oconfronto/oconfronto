...
if ($player->mana < $mana){
    ...
    if ($player->id == $luta['p_id']) {
        $db->execute("update `duels` set `p_magia`='6', `p_turnos`='3' where `id`=?", array($luta['id']));
    } else {
        $db->execute("update `duels` set `e_magia`='6', `e_turnos`='3' where `id`=?", array($luta['id']));
    }
    
    $db->execute("update `players` set `mana`=`mana`-? where `id`=?", array($mana, $player->id));
    array_unshift($duellog, "3, " . $player->username . ", defesa dupla");
}
?>